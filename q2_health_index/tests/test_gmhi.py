...
class TestUtilities(TestPluginBase):
    ...
    def test_healthy_states_correct_simple(self):
        ...
        _validate_and_extract_healthy_states(metadata, 'Healthy', 'rest',
                                             'N,Sick')
    ...
    def test_healthy_states_parameters_not_provided(self):
        metadata_file = self.get_data_path("input/metadata"
                                           "/simple_metadata.tsv")
        metadata = _load_metadata(qiime2.Metadata.load(metadata_file))
        with self.assertRaisesRegex(ValueError,  "healthy_column parameter "
                                                 "not provided!"):
            _validate_and_extract_healthy_states(metadata, None, 'Y', 'N,Sick')
        with self.assertRaisesRegex(ValueError, "healthy_states parameter "
                                                "not provided!"):
            _validate_and_extract_healthy_states(metadata, 'Col', None, 'Par')
        with self.assertRaisesRegex(ValueError, "non_healthy_states "
                                                "parameter not provided!"):
            _validate_and_extract_healthy_states(metadata, 'Col', 'Par', None)

    def test_healthy_states_wrong_healthy_column(self):
        with self.assertRaisesRegex(ValueError, "\'Col\' is not a column in "
                                                "your metadata."):
            metadata_file = self.get_data_path("input/metadata"
                                               "/simple_metadata.tsv")
            metadata = _load_metadata(qiime2.Metadata.load(metadata_file))
    ...
    def test_healthy_states_both_states_equal(self):
        with self.assertRaisesRegex(ValueError, 'healthy_states and '
            ...
            metadata_file = self.get_data_path("input/metadata"
                                               "/simple_metadata.tsv")
            metadata = _load_metadata(qiime2.Metadata.load(metadata_file))
            _validate_and_extract_healthy_states(metadata, 'Healthy',
    ...
    def test_healthy_states_healthy_state_wrong(self):
        with self.assertRaisesRegex(ValueError, 'Healthy state \'Par1\' is '
            ...
            metadata_file = self.get_data_path("input/metadata"
                                               "/simple_metadata.tsv")
            metadata = _load_metadata(qiime2.Metadata.load(metadata_file))
            _validate_and_extract_healthy_states(metadata, 'Healthy', 'Par1',
                                                 'Par2')
    ...
    def test_healthy_states_non_healthy_state_wrong(self):
        with self.assertRaisesRegex(ValueError, 'Non-healthy state \'Par2\' '
            ...
            metadata_file = self.get_data_path("input/metadata"
                                               "/simple_metadata.tsv")
            metadata = _load_metadata(qiime2.Metadata.load(metadata_file))
            _validate_and_extract_healthy_states(metadata, 'Healthy', 'Y',
                                                 'Par2')
